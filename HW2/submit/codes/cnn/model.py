...
class Model:
    ...
    def forward(self, is_train, reuse=None):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            tensor = self.x_
            # Your Conv Layer
            tensor = tf.layers.conv2d(tensor, 64, (3, 3), padding='same', name="conv2d1", reuse=reuse)
            ...
            # Your Conv Layer
            tensor = tf.layers.conv2d(tensor, 128, (3, 3), padding='same', name="conv2d2", reuse=reuse)
            # Your BN Layer: use batch_normalization_layer function
            tensor = batch_normalization_layer(tensor, "bn2", reuse, is_train)
            # Your Relu Layer
            tensor = tf.nn.relu(tensor)
            # Your Dropout Layer: use dropout_layer function
            tensor = dropout_layer(tensor, FLAGS.drop_rate, "dropout2", is_train=is_train)
            # Your MaxPool
            tensor = tf.layers.max_pooling2d(tensor, (2, 2), (2, 2), name="maxpooling2")
            # Your Linear Layer
            tensor = tf.layers.flatten(tensor, name="flatten1")
            tensor = tf.layers.dense(tensor, units=10, name='dense1', reuse=reuse)
